...
flowchart TB
    ...
    subgraph PIPE["판독 파이프라인 (배치)"]
        ...
        P3["④ 상태 분류 · 품질 게이트<br/><small>개방수면 / 개방수면 감소 / 개방수면 미발달</small>"]
        P0 --> P1 --> P2 --> P3
    end
...
비교했습니다. 습지 **58개소**, 짝지은 관측 **1,035건**입니다.

| 구분 | 표본 | 두 센서 판정 일치율 |
|---|---|---|
| 개방수면 | 996 | **0.976** |
| 개방수면 감소 (후방산란 증가형) | 46 | 0.870 |
| 개방수면 감소 (후방산란 감소형) | 7 | 1.000 |

개방수면 구간의 일치율 **0.976** 이 이 시스템이 실제로 검증한 값입니다.
감소 구간은 두 센서가 모두 0에 가까워 자동으로 일치하는 부분이 섞입니다.
...
따라가지 못합니다. 58개소 중 R² 0.5 이상은 **8개소**뿐입니다.

| 습지 | 표본 | 면적 R² | MAE (ha) |
|---|---|---|---|
| 주남저수지 | 12 | 0.953 | 16.6 |
| 광천천하구습지 | 11 | 0.928 | 11.8 |
| 탑정호 | 12 | 0.723 | 20.6 |
| 해평습지 | 18 | 0.000 | 12.5 |
| 금강호습지 | 12 | 0.007 | 11.6 |

> 해평습지를 보십시오. R² 는 0.000 인데 레이더 평균 219 ha 대 광학 평균 216 ha 입니다. **수준은 맞는데 변동이 없어서 R² 가 0 인 경우**입니다.
> R² 하나로 판정하면 이런 습지를 "판독 실패"로 잘못 분류합니다.

> **여러 습지를 묶은 상관은 읽으면 안 됩니다.** 크기가 다른 습지를 한데 넣으면
> "큰 습지는 둘 다 크다"는 자명한 사실이 높은 상관으로 나타납니다.
> 습지별 R² 중앙값이 0.071 인 같은 자료에서, 묶어서 계산하면 **0.952** 가 됩니다.
...
| 상태 분류 (개방수면 / 감소) | ✅ Implemented | **두 센서 일치율 0.976** (개방수면 구간, n=996 · 습지 58개소) |
| 품질 게이트 (`has_open_water`·`confidence`) | ✅ Implemented | 개방수면 미발달 습지를 별도 집계. 저신뢰를 화면에 표시 |
| 정확도 산출 프레임 | ✅ Implemented | 상태별·시간차별·임계별 층화 산출. `scripts/run_validation.py` 로 재현 |
| **개방수면 면적(ha) 절대값** | ⚠️ **부분 성립** | 수준은 치우침 없음(평균비 0.993)이나 개별 오차 큼(수면적의 10.7%). 시간 변동 미추적(습지별 R² 중앙 0.071). **측정값으로 제시하지 않음** |
| 풍파 보정 | ❌ **가설 기각** | 습지별 상관 부호가 뒤집힘. VV 통제 후 편상관 −0.14. `scripts/run_wind_check.py` |
| 감소 원인 판정 (VV −13 dB) | ❌ **검증 실패** | 전국 자료에서 특이도 0.10 · Youden J 0.085. 주지표를 **개방수면 감소 관측 비중**으로 교체하고 원인 구분은 실험 항목으로 강등 |
...
| 판독 습지 | **489개소** (습지-연도 545건) |
| 처리한 SAR 관측 | **16,314회** |
| 평균 재방문 주기 | 12.3일 |
| 개방수면 지표 성립 | **369개소** (75%) · 고신뢰 370 습지-연도 |
| 개방수면 감소 관측 습지 | **141개소** |
| 정본 습지 적재 | 2,479 / 2,704개소 |

정본 유형 구성: 하천습지 1,223 · 호수습지 574 · 산지습지 415 · 인공습지 267.

전국 스크리닝은 경계가 뚜렷한 호수·하천형 30 ha 이상 489개소를 2025년 1개년으로 돌렸으며,
대면적 습지 23개소는 기간을 분할 요청해 채웠습니다.
그중 8개소를 2018~2025년 다년으로 정밀 판독했습니다.
Sentinel-2 광학 교차검증은 개방수면 지표가 성립한 습지 **58개소**(짝지은 관측 1,035건)에서
수행했습니다.
