...
| 구분 | 표본 | 수면 유무 일치율 | 면적 상관 R² |
|---|---|---|---|
| 개방수면 | 100 | **0.930** | **0.083** |
| 식생피복 | 55 | 0.836 | 0.378 |
| 건조 의심 | 9 | 0.889 | 0.108 |

식생피복 구간은 두 센서가 모두 0에 가까워 자동으로 일치하므로, 판정이 실제로 시험되는 곳은
개방수면 구간입니다.

### 면적은 검증되지 않았습니다

임계값을 바꾸어도 R² 는 0.03~0.08 에 머물렀습니다 — 임계 선택의 문제가 아닙니다.
...
| 상태 분류 (개방수면/식생피복/건조) | ✅ Implemented | **두 센서 일치율 0.930** (개방수면 구간, n=100) |
...
| **개방수면 면적(ha) 절대값** | ❌ **검증 실패** | R² 0.083. 면적의 41~87%가 평균 후방산란으로 설명됨. **산출물에서 제외** |
...
| 전국 확대 판독 | ⏳ 진행 중 | 호수·하천습지 30 ha 이상 489개소 배치 실행 중 |
