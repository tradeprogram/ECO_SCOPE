...
출처 우선순위
    1. 국립생태원 정본  data/raw/nie_inland_wetlands/*.shp
       (공공데이터포털 `국립생태원_내륙습지 공간데이터 및 속성정보`, 내륙습지 2,704개소,
        EPSG:5186. 로그인이 필요해 자동 수집 불가 — 받아서 넣기만 하면 여기가 먼저 잡힌다.)
    2. OSM 부트스트랩  data/raw/wetlands_osm.geojson
...
def load(source: str = "auto", inland_only: bool = True, min_area_ha: float | None = None) -> gpd.GeoDataFrame:
    """레지스트리를 읽는다. source: "auto" | "nie" | "osm"."""
    if source in ("auto", "nie"):
        gdf = _load_nie()
...
def describe() -> dict:
    ...
    nie, osm = _load_nie(), _load_osm()
    info: dict = {"active": "nie" if nie is not None else ("osm" if osm is not None else None)}
    for key, gdf in (("nie", nie), ("osm", osm)):
        if gdf is None:
            info[key] = None
            continue
