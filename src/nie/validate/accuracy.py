...
def report(pairs: list[dict]) -> dict:
    ...
    return {
        ...
        "note": (
            ...
            "실제로 습지 60개소·짝지음 1,075건에서 묶은 R² 는 0.92 인데, "
            "습지별 R² 의 중앙값은 0.07 입니다. "
            "상관은 반드시 습지별(by_wetland)로 보십시오. "
            "묶어서 읽어도 되는 값은 표본 수와 수면 유무 일치율입니다. "
            "또 하나 — **낮은 R² 를 '면적이 틀렸다'로 읽지 마십시오.** R² 는 변동을 "
            "설명하는 비율이라 변동 자체가 작으면 값이 낮게 나옵니다. 수준이 맞는지는 "
            "평균비와 MAE 로 따로 봐야 합니다(by_wetland 의 mean_sar_ha / mean_opt_ha)."
        ),
