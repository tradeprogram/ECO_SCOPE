...
const 제안명 = "현장조사 우선순위를 위성으로 — 내륙습지 상시 판독 체계";
...
const 제안내용 = [
  ...
  { fig: "fig4_validation.png", ratio: 0.444, w: 430,
    cap: "[그림 3] 검증 결과 — 왼쪽은 수면 유무 판정의 두 센서 분할표(검증된 것), 오른쪽은 습지별 면적 결정계수 분포(검증되지 않은 것)." },
...
const 기대효과 = [
  ...
  ["(조사 자원의 배분) 국립생태원 현장조사는 작은 규모가 아닙니다. 전국자연환경조사만 해도 매년 조사원 570~600명이 투입되고 현지조사 여비·수당이 40~45억원입니다(나라장터 과업설명서, 2022~2026년). 본 제안은 이 비용을 줄이자는 것이 아니라, 같은 인력을 변화가 실제로 일어난 곳으로 보내자는 것입니다. 변화가 관측된 습지를 우선 후보로 제시하고, 개방수면이 발달하지 않아 이 방식의 대상이 아닌 120개소는 미리 걸러냅니다.", 1],
