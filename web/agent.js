...
` +
        `계절로는 구분됩니다. 생장기 감소 관측의 광학 구성은 식생 55% · 비식생 15%, ` +
        `비생장기는 14% · 28% 입니다. 집계 수준에서 여름철 감소는 대체로 식생에 의한 것으로 봅니다.`;
      if (s.optical.length) {
        const hi = [...s.optical].sort((a, b) => b.ndvi - a.ndvi)[0];
        out += `
    ...
    if (has("저신뢰", "신뢰", "믿을", "정확", "검증", "맞나", "오차")) {
      if (!s.has_open_water) {
        return `${s.name}${josa(s.name, "은", "는")} 연중 최대 개방수면율이 5% 미만이므로 개방수면 지표를 적용하지 않았습니다.\n` +
          `삼림습지·초본습지와 같이 SAR 로 관측할 수면이 존재하지 않는 유형으로 판단됩니다. 저신뢰가 아니라 개방수면 미발달에 해당합니다.`;
      }
      if (s.confidence === "low") {
